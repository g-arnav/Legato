...
void DistortionAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        {
            og = channelData[sample];
            channelData[sample] *= drive;
            //channelData[sample] = range / M_PI_2 * atan(channelData[sample]);
            channelData[sample] = fmax(fmin(channelData[sample], range), -range);
            channelData[sample] = channelData[sample] * mix + og * (1-mix);
